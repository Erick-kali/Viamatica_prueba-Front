...
export class AuthService {
  ...
  private catalogoUrl = `${this.baseUrl}/catalogos`;  // URL para el endpoint de catálogo (estados)
  ...
  /**
   * Obtiene el catálogo de estados (u otras categorías) desde el backend.
   * @returns Observable<Catalogo[]>
   */
  obtenerCatalogo(): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>(this.catalogoUrl).pipe(
      catchError(this.handleError<Catalogo[]>('obtenerCatalogo', []))  // Manejo de error
    );
  }
